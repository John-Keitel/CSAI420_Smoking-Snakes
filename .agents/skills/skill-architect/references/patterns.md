...
This document details the five proven patterns for skill architecture. Read this when deciding how to structure a
skill's workflow during the Architecture phase.
...
**Use when:** Users need multi-step processes executed in a specific order, where each step depends on the previous one.

**Problem-first framing:** "I need to onboard a new customer" → Skill orchestrates the right calls in the right
sequence.
...
Call tool: `tool_name` Parameters: [what's needed] Validation: [how to know it succeeded] On failure: [what to do]

### Step 2: [Action]

Depends on: Step 1 (uses [specific output]) Call tool: `tool_name` Parameters: [include output from Step 1] Validation:
[check]
...
**Use when:** Workflows span multiple external services, each connected via its own MCP server.
...
2. [Action using Service A tools] Output: [data needed by Phase 2]
...
2. [Action using Service B tools] Output: [data needed by Phase 3]
...
Run validation: `scripts/check_quality.py` Criteria:
...
**Use when:** The same goal can be achieved with different tools depending on the input or context.
...
IF [condition 1]: → Use [Tool/Approach A] Rationale: [why this is better for this case] ELIF [condition 2]: → Use
[Tool/Approach B] Rationale: [why] ELSE: → Use [Tool/Approach C] (default)
...
**Use when:** The skill's value comes from specialized knowledge, not just tool orchestration.
...
IF any rule fails: → [Escalation or alternative path] → Document the failure
...
- **Sequential + Domain Intelligence:** Follow steps in order, but embed expert checks at critical points (e.g.,
  compliance check before payment)
- **Multi-MCP + Iterative:** Coordinate across services, then refine the combined output
- **Context-Aware + Sequential:** Choose the right tool first, then follow a sequential workflow specific to that tool
- **Domain Intelligence + Iterative:** Apply domain rules, generate output, review against domain criteria, refine

When combining, identify the PRIMARY pattern (the one that shapes the overall flow) and SECONDARY patterns (the ones
that apply within specific steps).
