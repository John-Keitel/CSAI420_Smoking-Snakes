...
> never write or edit source code during planning. See `.cursor/rules/spec-driven-flow.mdc`
> ("STOP - Planning and execution are two separate sessions"). The `EXECUTE` column below and the
> Execute/Verifier instructions apply only in that separate `sdd-execute-jira` session, not here.
