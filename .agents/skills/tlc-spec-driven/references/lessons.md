...
After distilling, if `validation.md` contained any FAIL, surviving mutant, spec-precision gap, or SPEC*DEVIATION but you recorded zero lessons, state plainly in chat:*"Validation had signal X but no lesson was recorded — recording now / here's why it's out of scope."\_ Silent skipping is how the file dies.
