...
describe('POST /api/coach/chat integration boundaries', () => {
    ...
    it('Path 200: upserts a flagged session when the coach escalates', async () => {
        ...
        const { POST, upsertFlaggedSessionOnEscalateMock, notifyModeratorsHighRiskMock, saveAiResponseMock } = await loadRoute({
            sessionCheck: { ok: true, user: { id: 'user-123', email: 'patient@example.com', type: 'standard' } },
            coachResult,
        ...
        expect(response.status).toBe(200);
        expect(saveAiResponseMock).toHaveBeenCalledWith('session-escalate', coachResult.responseText, {
            escalate: true,
